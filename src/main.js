(function() {
    ...
    if (window.stackitFavoritesInitialized) {
        return;
    }
    window.stackitFavoritesInitialized = true;
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .favorite-star-btn { position: absolute; right: 1rem; top: 50%; transform: translateY(-50%); cursor: pointer; opacity: 0; transition: opacity 0.2s ease-in-out; z-index: 10; padding: 4px; border-radius: 4px; }
    ...
    function renderFavoritesSection() {
        ...
        const listItemsHTML = favorites.map(link => {
            const finalHref = projectId ? link.urlPattern.replace('{{PROJECT_ID}}', projectId) : link.urlPattern;
            return `
                <li class="nav-item ng-star-inserted" data-link-id="${link.id}">
    ...
    function discoverAndMarkNavItems() {
        const navContainer = document.querySelector(navMenuSelector);
